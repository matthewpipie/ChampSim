...
void pythia_4kb::prefetcher_initialize()
{
  init_knobs();
...
uint32_t pythia_4kb::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                          uint32_t metadata_in)
{
  uint64_t address = addr.to<uint64_t>();
  uint64_t pc = ip.to<uint64_t>();

  uint64_t page = address >> PYTHIA_4KB_LOG2_PAGE_SIZE;
  uint32_t offset = (uint32_t)((address >> LOG2_BLOCK_SIZE) & (PYTHIA_4KB_BLOCKS_PER_PAGE - 1));
...
uint32_t pythia_4kb::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  register_fill(addr.to<uint64_t>());
  return 0;
}

void pythia_4kb::prefetcher_cycle_operate() {}

void pythia_4kb::prefetcher_final_stats() {}
