...
void pythia_4kb::update_global_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address) { /* @rbera TODO: implement */ }

Scooby_STEntry* pythia_4kb::update_local_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address)
{
  stats.st.lookup++;
...
//----------------------------------------------------//
uint32_t pythia_4kb::predict(uint64_t base_address, uint64_t page, uint32_t offset, State* state, std::vector<uint64_t>& pref_addr)
{
  ...
  if (Actions[action_index] != 0) {
    ...
    if (predicted_offset >= 0 && predicted_offset < PYTHIA_4KB_BLOCKS_PER_PAGE) /* falls within the 4KB page */
    {
      addr = (page << PYTHIA_4KB_LOG2_PAGE_SIZE) + (predicted_offset << LOG2_BLOCK_SIZE);
...
//----------------------------------------------------//
bool pythia_4kb::track(uint64_t address, State* state, uint32_t action_index, Scooby_PTEntry** tracker)
{
...
//----------------------------------------------------//
uint32_t pythia_4kb::get_dyn_pref_degree(float max_to_avg_q_ratio, uint64_t page, int32_t action)
{
...
//----------------------------------------------------//
void pythia_4kb::gen_multi_degree_pref(uint64_t page, uint32_t offset, int32_t action, uint32_t pref_degree, std::vector<uint64_t>& pref_addr)
{
  ...
  if (action != 0) {
    for (uint32_t degree = 2; degree <= pref_degree; ++degree) {
      ...
      if (predicted_offset >= 0 && predicted_offset < PYTHIA_4KB_BLOCKS_PER_PAGE) {
        addr = (page << PYTHIA_4KB_LOG2_PAGE_SIZE) + (predicted_offset << LOG2_BLOCK_SIZE);
        pref_addr.push_back(addr);
...
//----------------------------------------------------//
void pythia_4kb::reward(uint64_t address)
{
...
//----------------------------------------------------//
void pythia_4kb::reward(Scooby_PTEntry* ptentry)
{
...
//----------------------------------------------------//
void pythia_4kb::assign_reward(Scooby_PTEntry* ptentry, RewardType type)
{
...
//----------------------------------------------------//
int32_t pythia_4kb::compute_reward(Scooby_PTEntry* ptentry, RewardType type)
{
...
//----------------------------------------------------//
void pythia_4kb::train(Scooby_PTEntry* curr_evicted, Scooby_PTEntry* last_evicted)
{
...
//----------------------------------------------------//
void pythia_4kb::register_fill(uint64_t address)
{
...
std::vector<Scooby_PTEntry*> pythia_4kb::search_pt(uint64_t address, bool search_all)
{
...
int32_t pythia_4kb::getAction(uint32_t action_index)
{
  assert(action_index < Actions.size());
  return Actions[action_index];
}

void pythia_4kb::track_in_st(uint64_t page, uint32_t pred_offset, int32_t pref_offset)
{
  auto st_index = find_if(signature_table.begin(), signature_table.end(), [page](Scooby_STEntry* stentry) { return stentry->page == page; });
  if (st_index != signature_table.end()) {
...
bool pythia_4kb::is_high_bw(uint8_t bw_level) { return bw_level >= PYTHIA::high_bw_thresh ? true : false; }
