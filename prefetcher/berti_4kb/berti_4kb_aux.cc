...
uint64_t berti_4kb::l1d_get_latency(uint64_t cycle, uint64_t cycle_prev)
{
...
int berti_4kb::l1d_calculate_stride(uint64_t prev_offset, uint64_t current_offset)
{
  int stride;
...
void berti_4kb::l1d_init_current_pages_table()
{
...
uint64_t berti_4kb::l1d_get_current_pages_entry(uint64_t page_addr)
{
  for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_ENTRIES; i++) {
...
void berti_4kb::l1d_update_lru_current_pages_table(uint64_t index)
{
...
uint64_t berti_4kb::l1d_get_lru_current_pages_entry()
{
...
int berti_4kb::l1d_get_berti_current_pages_table(uint64_t index, uint64_t& ctr)
{
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
...
void berti_4kb::l1d_add_current_pages_table(uint64_t index, uint64_t page_addr, uint64_t ip, uint64_t offset)
{
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
...
uint64_t berti_4kb::l1d_update_demand_current_pages_table(uint64_t index, uint64_t offset)
{
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  l1d_current_pages_table[index].u_vector |= (uint64_t)1 << offset;
...
void berti_4kb::l1d_add_berti_current_pages_table(uint64_t index, int b)
{
  assert(b != 0);
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_NUM_BERTI; i++) {
...
bool berti_4kb::l1d_requested_offset_current_pages_table(uint64_t index, uint64_t offset)
{
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  return l1d_current_pages_table[index].u_vector & ((uint64_t)1 << offset);
}

void berti_4kb::l1d_remove_current_table_entry(uint64_t index)
{
...
void berti_4kb::l1d_init_prev_requests_table()
{
...
uint64_t berti_4kb::l1d_find_prev_request_entry(uint64_t pointer, uint64_t offset)
{
  for (int i = 0; i < L1D_PREV_REQUESTS_TABLE_ENTRIES; i++) {
...
void berti_4kb::l1d_add_prev_requests_table(uint64_t pointer, uint64_t offset, uint64_t cycle)
{
...
void berti_4kb::l1d_reset_pointer_prev_requests(uint64_t pointer)
{
  for (int i = 0; i < L1D_PREV_REQUESTS_TABLE_ENTRIES; i++) {
...
uint64_t berti_4kb::l1d_get_latency_prev_requests_table(uint64_t pointer, uint64_t offset, uint64_t cycle)
{
...
void berti_4kb::l1d_get_berti_prev_requests_table(uint64_t pointer, uint64_t offset, uint64_t cycle, int* b)
{
  int my_pos = 0;
...
void berti_4kb::l1d_init_prev_prefetches_table()
{
...
uint64_t berti_4kb::l1d_find_prev_prefetch_entry(uint64_t pointer, uint64_t offset)
{
  for (int i = 0; i < L1D_PREV_PREFETCHES_TABLE_ENTRIES; i++) {
...
void berti_4kb::l1d_add_prev_prefetches_table(uint64_t pointer, uint64_t offset, uint64_t cycle)
{
  // First find for coalescing
...
void berti_4kb::l1d_reset_pointer_prev_prefetches(uint64_t pointer)
{
  for (int i = 0; i < L1D_PREV_PREFETCHES_TABLE_ENTRIES; i++) {
...
void berti_4kb::l1d_reset_entry_prev_prefetches_table(uint64_t pointer, uint64_t offset)
{
...
uint64_t berti_4kb::l1d_get_and_set_latency_prev_prefetches_table(uint64_t pointer, uint64_t offset, uint64_t cycle)
{
  uint64_t index = l1d_find_prev_prefetch_entry(pointer, offset);
  if (index == L1D_PREV_PREFETCHES_TABLE_ENTRIES)
    return 0;
...
uint64_t berti_4kb::l1d_get_latency_prev_prefetches_table(uint64_t pointer, uint64_t offset)
{
  uint64_t index = l1d_find_prev_prefetch_entry(pointer, offset);
  if (index == L1D_PREV_PREFETCHES_TABLE_ENTRIES)
    return 0;
...
void berti_4kb::l1d_init_record_pages_table()
{
...
uint64_t berti_4kb::l1d_get_lru_record_pages_entry()
{
...
void berti_4kb::l1d_update_lru_record_pages_table(uint64_t index)
{
...
void berti_4kb::l1d_add_record_pages_table(uint64_t index, uint64_t page_addr, uint64_t vector, uint64_t first_offset, int b)
{
  assert(index < L1D_RECORD_PAGES_TABLE_ENTRIES);
...
uint64_t berti_4kb::l1d_get_entry_record_pages_table(uint64_t page_addr, uint64_t first_offset)
{
...
uint64_t berti_4kb::l1d_get_entry_record_pages_table(uint64_t page_addr)
{
  uint64_t trunc_page_addr = page_addr & L1D_TRUNCATED_PAGE_ADDR_MASK;
  for (int i = 0; i < L1D_RECORD_PAGES_TABLE_ENTRIES; i++) {
...
void berti_4kb::l1d_copy_entries_record_pages_table(uint64_t index_from, uint64_t index_to)
{
...
// IP TABLE
//------------------------------------------//
void berti_4kb::l1d_init_ip_table()
{
...
// TABLE MOVEMENTS
//------------------------------------------//
void berti_4kb::l1d_record_current_page(uint64_t index_current)
{
