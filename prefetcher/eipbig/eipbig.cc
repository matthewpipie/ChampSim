...
#define L1I_ENTANGLED_MAX_FORMATS 7lu

// HISTORY TABLE

#define L1I_HIST_TABLE_ENTRIES 1072lu
#define L1I_HIST_TABLE_MASK (L1I_HIST_TABLE_ENTRIES - 1)
#define L1I_BB_MERGE_ENTRIES 4lu
#define L1I_HIST_TAG_BITS 58lu
...
static l1i_hist_entry l1i_hist_table[NUM_CPUS][L1I_HIST_TABLE_ENTRIES];
static uint64_t l1i_hist_table_head[NUM_CPUS]; // log_2 (L1I_HIST_TABLE_ENTRIES)
static uint64_t l1i_hist_table_head_time[NUM_CPUS]; // 64 bits

static void l1i_init_hist_table() {
...
static uint64_t l1i_find_hist_entry(uint64_t line_addr) {
...
static void l1i_add_hist_table(uint64_t line_addr) {
...
static void l1i_add_bb_size_hist_table(uint64_t line_addr, uint32_t bb_size) {
  uint64_t index = l1i_find_hist_entry(line_addr);
  l1i_hist_table[l1i_cpu_id][index].bb_size = bb_size & L1I_MERGE_BBSIZE_MAX_VALUE;
}

static uint32_t l1i_find_bb_merge_hist_table(uint64_t line_addr) {
  uint64_t tag = line_addr & L1I_HIST_TAG_MASK; 
  for (uint32_t count = 0, i = (l1i_hist_table_head[l1i_cpu_id] + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES; count < L1I_HIST_TABLE_ENTRIES; count++, i = (i + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES) {
...
static uint64_t l1i_get_bere_hist_table(uint64_t line_addr, uint64_t latency, uint32_t skip = 0) {
  uint64_t tag = line_addr & L1I_HIST_TAG_MASK; 
  assert(tag);
...
static l1i_timing_mshr_entry l1i_timing_mshr_table[NUM_CPUS][L1I_TIMING_MSHR_SIZE];
static l1i_timing_cache_entry l1i_timing_cache_table[NUM_CPUS][L1I_SET][L1I_WAY];

static void l1i_init_timing_tables() {
...
static uint64_t l1i_find_timing_mshr_entry(uint64_t line_addr) {
  for (uint32_t i = 0; i < L1I_TIMING_MSHR_SIZE; i++) {
...
static uint64_t l1i_find_timing_cache_entry(uint64_t line_addr) {
...
static uint32_t l1i_get_invalid_timing_mshr_entry() {
  for (uint32_t i = 0; i < L1I_TIMING_MSHR_SIZE; i++) {
...
static uint32_t l1i_get_invalid_timing_cache_entry(uint64_t line_addr) {
...
static void l1i_add_timing_entry(uint64_t line_addr, uint64_t bere_line_addr) {
...
static void l1i_invalid_timing_mshr_entry(uint64_t line_addr) {
...
static void l1i_move_timing_entry(uint64_t line_addr) {
...
// returns if accessed
static bool l1i_invalid_timing_cache_entry(uint64_t line_addr, uint64_t &bere_line_addr) {
  uint32_t set = line_addr % L1I_SET;
...
static void l1i_access_timing_entry(uint64_t line_addr) {
  uint32_t index = l1i_find_timing_mshr_entry(line_addr);
...
static bool l1i_is_accessed_timing_entry(uint64_t line_addr) {
  uint32_t index = l1i_find_timing_mshr_entry(line_addr);
  if (index < L1I_TIMING_MSHR_SIZE) {
...
static bool l1i_completed_request(uint64_t line_addr) {
  return l1i_find_timing_cache_entry(line_addr) < L1I_WAY;
}

static bool l1i_ongoing_request(uint64_t line_addr) {
  return l1i_find_timing_mshr_entry(line_addr) < L1I_TIMING_MSHR_SIZE;
}

static bool l1i_ongoing_accessed_request(uint64_t line_addr) {
  uint32_t index = l1i_find_timing_mshr_entry(line_addr);
  if (index == L1I_TIMING_MSHR_SIZE) return false;
  return l1i_timing_mshr_table[l1i_cpu_id][index].accessed;
}

static uint64_t l1i_get_latency_timing_mshr(uint64_t line_addr) {
  uint32_t index = l1i_find_timing_mshr_entry(line_addr);
...
static uint32_t L1I_ENTANGLED_FORMATS[L1I_ENTANGLED_MAX_FORMATS] = {58, 28, 18, 13, 10, 8, 6};
#define L1I_ENTANGLED_NUM_FORMATS 6

static uint32_t l1i_get_format_entangled(uint64_t line_addr, uint64_t entangled_addr) {
...
static uint64_t l1i_extend_format_entangled(uint64_t line_addr, uint64_t entangled_addr, uint32_t format) {
...
static uint64_t l1i_compress_format_entangled(uint64_t entangled_addr, uint32_t format) {
...
static l1i_entangled_entry l1i_entangled_table[NUM_CPUS][L1I_ENTANGLED_TABLE_SETS][L1I_ENTANGLED_TABLE_WAYS];
static uint32_t l1i_entangled_fifo[NUM_CPUS][L1I_ENTANGLED_TABLE_SETS]; // log2(L1I_ENTANGLED_TABLE_WAYS) * L1I_ENTANGLED_TABLE_SETS bits

static void l1i_init_entangled_table() {
...
static uint32_t l1i_get_way_entangled_table(uint64_t line_addr) {
...
static void l1i_add_entangled_table(uint64_t line_addr, uint64_t entangled_addr) {
  uint64_t tag = (line_addr >> L1I_ENTANGLED_TABLE_INDEX_BITS) & L1I_TAG_MASK; 
  uint32_t set = line_addr % L1I_ENTANGLED_TABLE_SETS;
...
static bool l1i_avail_entangled_table(uint64_t line_addr, uint64_t entangled_addr, bool insert_not_present) {
  uint32_t set = line_addr % L1I_ENTANGLED_TABLE_SETS;
  uint32_t way = l1i_get_way_entangled_table(line_addr);
...
static void l1i_add_bbsize_table(uint64_t line_addr, uint32_t bb_size) {
  uint64_t tag = (line_addr >> L1I_ENTANGLED_TABLE_INDEX_BITS) & L1I_TAG_MASK; 
  uint32_t set = line_addr % L1I_ENTANGLED_TABLE_SETS;
  uint32_t way = l1i_get_way_entangled_table(line_addr);
  if (way == L1I_ENTANGLED_TABLE_WAYS) {
...
static uint64_t l1i_get_entangled_addr_entangled_table(uint64_t line_addr, uint32_t index_k) {
  uint32_t set = line_addr % L1I_ENTANGLED_TABLE_SETS;
  uint32_t way = l1i_get_way_entangled_table(line_addr);
  if (way < L1I_ENTANGLED_TABLE_WAYS) {
...
static uint32_t l1i_get_bbsize_entangled_table(uint64_t line_addr) {
  uint32_t set = line_addr % L1I_ENTANGLED_TABLE_SETS;
  uint32_t way = l1i_get_way_entangled_table(line_addr);
  if (way < L1I_ENTANGLED_TABLE_WAYS) {
...
static void l1i_update_confidence_entangled_table(uint64_t line_addr, uint64_t entangled_addr, bool accessed) {
  uint32_t set = line_addr % L1I_ENTANGLED_TABLE_SETS;
  uint32_t way = l1i_get_way_entangled_table(line_addr);
  if (way < L1I_ENTANGLED_TABLE_WAYS) {
    for (uint32_t k = 0; k < L1I_MAX_ENTANGLED_PER_LINE; k++) {
...
static l1i_xpq_entry l1i_xpq[NUM_CPUS][L1I_XPQ_ENTRIES];
static uint64_t l1i_xpq_head[NUM_CPUS]; // log_2 (L1I_XPQ_ENTRIES)

static void l1i_init_xpq() {
...
static void l1i_add_xpq(uint64_t line_addr, uint64_t entangled_addr, uint32_t bb_size) {
  assert(bb_size > 0);
