...
workloads = list(map(lambda y: y.name, filter(lambda x: x.is_dir(), Path(BASE_DIR).glob("*"))))
...
for executable_nopath in executables_nopath:
    ...
    OUT_DIR = f"results_spec/{executable_name}"
    for workload in workloads:
        ...
        for i, trace_file_batch in enumerate(list(grouper(trace_files, N_CORES_PER_PROCESS, incomplete="strict"))):
            ...
            commands.append({"command": command, "outfile": str(outfile.absolute())})
            print("appending:", (command, outfile))
...
def work(command_outfile):
    ...
    print("Launching " + Path(command_outfile["outfile"]).name + " with command: ", command[0])
    if ECHO_ONLY:
